...
def formcontacts():
    ...
    with open("data.json") as data:
        ...
        # Set headers
        headers = {"Content-Type": "application/json", "X-API-Key": "post-key"}

        # Make request
        res = requests.post(
            os.environ["MAIN_API"] + "/contacts", json=file_data, headers=headers
        )
        # Return request
        return res.json()
...
def getVCardCache():
    ...
    expired_docs = collection2.find(
        {"timestamp": {"$lt": datetime.utcnow() - timedelta(days=10)}}
    )
...
def getVCardIdCache(id):
    ...
    # If you don't find id, then make request to mainAPI
        ...
        # Send request with headers
        res = requests.get(
            os.environ["MAIN_API"] + f"/contacts/{id}/vcard", headers=headers
        )
